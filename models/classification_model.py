...
class ClassificationModel:
    def __init__(self, model_path: str, class_names: list, device: str = "cpu"):
        ...
        self.device = device
        # Use pretrained backbone; override final layer for environment classes
        self.model = models.resnet18(pretrained=True)
        num_ftrs = self.model.fc.in_features
        self.model.fc = torch.nn.Linear(num_ftrs, num_classes)
        # Load local weights if available, else warn and use pretrained weights
        if os.path.exists(model_path):
            self.model.load_state_dict(torch.load(model_path, map_location=self.device))
        else:
            print(f"Warning: classifier weights not found at {model_path}, using ImageNet pretrained weights")
